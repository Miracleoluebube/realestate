...
function initStreetView() {
  ...
  const Nigeria= {lat: 6.4345328, lng:3.4585664 };
...
function initMap() {
  ...
  const sanFrancisco = {lat: 6.4345328, lng: 3.4585664};
  // The map, centered at San Francisco
  const map = new google.maps.Map(
      document.getElementById('map'), {zoom: 12, center: Lagos});
  // The marker, positioned at San Francisco
  const marker = new google.maps.Marker({position: Lagos, map: map});
}
